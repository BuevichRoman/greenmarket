...
Следствие для миграции 012 (`is_published` DEFAULT `TRUE` → `FALSE`): на путь
через ORM она не влияет вообще — SQLAlchemy всё равно шлёт значение явно, а
`SellerProductRepository.create()` требует `is_published` обязательным
аргументом (дефолта у него больше нет, иначе безопасное значение существовало бы
только в схеме). Новый DB-side дефолт защищает вставки мимо приложения: руками
через phpMyAdmin, сидером, будущей миграцией. Нормативное правило —
`docs/02-domain/Publication_Model.md`, раздел «Видимость предложения в Buyer
Catalog».
