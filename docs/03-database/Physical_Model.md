...
**`name`** — `UNIQUE (name)` (`uk_Product_name`, миграция 019). Уникальность глобальная, а не внутри группы: покупателю группа на плитке каталога не показывается, поэтому две позиции с одним именем в разных группах («Яблоки» во Фруктах и в Сухофруктах) для него неразличимы, а модератор видит в поиске по справочнику две одинаковые строки и привязывает предложение к любой из них. Различать товары должно наименование («Яблоки» и «Яблоки сушёные»). Collation таблицы регистронезависимая, поэтому «ЧЕРЕМША» тёзкой «Черемши» тоже не станет. То же правило проверяет Admin API до записи (DDL_Specification.md: UNIQUE не заменяет проверку бизнес-логики приложения).
