...
- привязку состояний к экранам [Customer_UI_UX.md](Customer_UI_UX.md).
...
| Состояние | Экран | Описание |
|---|---|---|
| `Initial` | Главная | Точка входа, до первого запроса к API |
| `Loading` | — | Ожидание ответа API. Общее для всех запросов (категории/поиск-каталог/карточка товара) — какой конкретно запрос выполняется, не моделируется отдельным состоянием, это деталь реализации ViewModel |
...
| `ProductOpened` | Карточка товара | Предложения продавцов по товару получены |
...
`ProductOpened` не имеет исходящего перехода дальше по happy path — соответствует закрытому решению [Customer_UI_UX.md](Customer_UI_UX.md) (раздел 3, шаг 6): сценарий Stage 1 заканчивается на выборе продавца.
