...
- `GET /api/v1/catalog/products/{id}` — карточка товара: Product, его товарная группа (`group_id`, `group_name`), список SellerProduct, цены, остатки, фотографии. Группа берётся у товарной позиции, а не у предложения продавца: продавец выбирает товар, группа определена товаром. Каждое предложение дополнительно содержит `origin_country` (строка или `null`) и `supply_date` (дата `ГГГГ-ММ-ДД` или `null`) — страну происхождения и дату завоза партии, как их указал продавец в книге (шаблон 2.2). `null` означает «продавец не указал» и является штатным состоянием: колонки необязательные, а у книг шаблона 2.1 их нет вовсе.
- `GET /api/v1/catalog/sellers/{id}` — карточка продавца: `{"seller_id": int, "name": str, "market": {"id": int, "name": str, "address": str, "latitude": decimal|null, "longitude": decimal|null}|null, "row": str|null, "place": str|null, "working_hours": str|null, "short_description": str|null, "phone": str|null, "whatsapp": str|null}`. `market` — сам рынок, а не его идентификатор: покупателю нужны название, адрес и точка для экрана «Карта». `null` — продавец рынок не выбрал либо выбранный рынок закрыт; рынок без снятых координат отдаётся с `latitude`/`longitude` равными `null`, а не скрывается. Состав — [Seller_Profile.md](../02-domain/Seller_Profile.md), §9. Деактивированный или несуществующий продавец — `404` `NOT_FOUND`: покупателю неактивный продавец не показывается (§9), и различать эти два случая незачем — иначе по кодам ответа можно было бы перебором выяснить, кто из продавцов деактивирован. Код `NOT_FOUND`, а не `SELLER_NOT_FOUND` как в Seller/Admin API, — сознательно: в Catalog API все 404 одинаковы (см. `GET /catalog/products/{id}`), покупателю не полагается различать причину. Учётный телефон платформы (`users.phone`) здесь не появляется никогда — отдаётся только то, что продавец сохранил сам.
...
- `GET /api/v1/seller/markets?access_token=...` — справочник рынков для выпадающего списка формы профиля: `{"markets": [{"id": int, "name": str, "address": str}]}`. Только открытые рынки — закрытый выбрать нельзя, поэтому он и не предлагается. Координаты форме не отдаются: она их не показывает и не правит, рынки ведёт администратор.
...
- `GET /api/v1/admin/markets` — справочник рынков: `{"markets": [{"id": int, "name": str, "address": str, "latitude": decimal|null, "longitude": decimal|null, "is_active": bool}]}`. Включая закрытые — закрытый рынок иначе нечем вернуть в работу (то же правило, что у товарных групп).
- `POST /api/v1/admin/markets` — тело `{"name": str, "address": str, "latitude": decimal|null, "longitude": decimal|null}`, ответ `201` — объект рынка. Координаты необязательны: рынок заводится по названию и адресу, точку снимают позже. Широта вне `[-90, 90]` и долгота вне `[-180, 180]` — `422`: это опечатка, а не точка на Земле, и БД такое приняла бы молча.
